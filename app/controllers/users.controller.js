...
module.exports = {
    ...
    getNames: function (req, res, next) {
        ...
        Users.find({}, 'name email', function (err, result) {
            if (err) return next(err);
            console.log(result);
            res.status(200).json({ status: 1, message: null, data: result });
        });
    ...
    save: function (req, res, next) {
        console.log("innername", req.body.name);
        var inneremail = (req.body.email).toLowerCase();
        if(req.body.name==undefined){res.json({ message: "the name cannot be blank" });
        }else{var innername = (req.body.name).toUpperCase();}
        ...
        Users.findOne({ name: innername}, function (err, existingName) {
            if (existingName) {
                ...
                Users.findOne({ phone: innerphone}, function (err, existingPhone) {
                    if (existingPhone) {
